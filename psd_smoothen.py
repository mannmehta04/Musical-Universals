...
class RasyaPSDAgent:
    ...
    def plot_single_rasya(self, rasya_code, max_frame_traces=500, save_path=None):
        ...
        plt.semilogy(f, avg_psd, color='black', linewidth=2, label=f"{self.rasya_map[rasya_code]} - Normalized Avg PSD")

        plt.xlabel("Normalized Frequency (Fn = F/Fmax)")
        plt.ylabel("Normalized Amplitude (An = A/Amax)")
        plt.title(f"Normalized PSD Curves: {self.rasya_map[rasya_code]}")
        plt.legend()
    ...
    def plot_all_rasyas(self, max_frame_traces=500, save_path=None):
        ...
        plt.xlabel("Normalized Frequency (Fn = F/Fmax)")
        plt.ylabel("Normalized Amplitude (An = A/Amax)")
        plt.title("Normalized Average PSDs for All Rasyas (Including Combined)")
        plt.legend()
        plt.grid(True, which='both', ls='--', alpha=0.4)
        plt.tight_layout()
        if save_path:
...
if __name__ == "__main__":
    dataset_path = r"personal_dataset"
    ...
    all_results = agent.plot_all_rasyas(
        ...
        save_path="gujarati_rasya_all_overlay_normalized.png"
    )
